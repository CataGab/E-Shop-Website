...
public class ShopDB {
    ...
    public void order(Basket basket , String customer) {
        try {
            ...
            while (i.hasNext()) {
                Product product = i.next().getKey();
                // and place the order for each one
                order( basket, product, orderId, customer );
            }
        }
        catch(Exception e) {
            e.printStackTrace();
...
// here I added all the information after a customer has bought something into a text file
    private void order(Basket bas, Product p, String orderId, String customer) throws Exception {
        double t= bas.items.get(p);
        Statement state = con.createStatement();
        String q= "INSERT INTO ORDERS VALUES('" + p.PID + "','" + orderId + "','" + customer + "','" + t +  "','" + p.price + "')";
        System.out.println(q);
        ResultSet result = state.executeQuery(q);
        System.out.println("The Order has been added: "+ q);
